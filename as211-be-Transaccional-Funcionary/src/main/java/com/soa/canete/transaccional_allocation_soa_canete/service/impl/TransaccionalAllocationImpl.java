...
public class TransaccionalAllocationImpl implements TransaccionalAllocationService {
    ...
    @Override
    public Mono<TransaccionalAllocationResponseDto> saveNewAsignamentTutor(TransaccionalAllocationRequestDto request) {
        return this.transaccionalAllocationRepository.findById_adolescente(request.getId_adolescente())
                .switchIfEmpty(Mono.error(new ResourceNotFoundException("El identificador: " + request.getId_adolescente() + "no fue encontrado.")))
                .flatMap(data -> {
                    TransaccionalAllocationRequestDto prueba = new TransaccionalAllocationRequestDto(
                            data.getDescription(),
                            "I",
                            data.getId_adolescente(),
                            data.getId_funcionary(),
                            data.getStart_date(),
                            data.getRegister_date());
                    return this.transaccionalAllocationRepository.save(toModel(prueba,data.getId_funcionaryteend()));
                })
                .flatMap(savedata ->{
                    request.setEstado("A");
                    return this.transaccionalAllocationRepository.save(toModel(request));
